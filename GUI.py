...
class GUIWindow:
    ...
    def setupInputFields(self):
        ...
        self.selectWearBox = ttk.Combobox(
            self.root, textvariable=self.selectedWear, state="readonly"
        )
        self.selectWearBox["values"] = [
            "Factory New (FN)",
            "Minimal Wear (MW)",
            "Field-Tested (FT)",
            "Well-Worn (WW)",
        ...
        self.selectRarityBox = ttk.Combobox(
            self.root, textvariable=self.selectedRarity, state="readonly"
        )
        self.selectRarityBox["values"] = [
            "Mil-Spec (Blue)",
            "Restricted (Purple)",
            "Classified (Pink)",
            "Covert (Red)",
        ...
        self.selectWeaponTypeBox = ttk.Combobox(
            self.root, textvariable=self.selectedWeaponType, state="readonly"
        )
        self.selectWeaponTypeBox["values"] = [
            "---PISTOLS---",
            "CZ75-Auto",
            "Desert Eagle",
            "Dual Berettas",
    ...
    def setupMetrics(self):
        tk.Label(self.root, text="MODEL METRICS").grid(
            row=0, column=10, padx=10, pady=10, sticky="w"
        )
        tk.Label(
            self.root,
            text=f"MAE: {self.metrics['MAE']}",
            background="gray15",
            foreground="white",
        ).grid(row=1, column=10, padx=10, sticky="w")
        tk.Label(
            self.root,
            text=f"RMSE: {self.metrics['RMSE']}",
            background="gray15",
            foreground="white",
        ).grid(row=2, column=10, padx=10, sticky="w")
        tk.Label(
            self.root,
            text=f"MAPE: {self.metrics['MAPE']}",
            background="gray15",
            foreground="white",
        ).grid(row=3, column=10, padx=10, sticky="w")
        tk.Label(
            self.root,
            text=f"R2: {self.metrics['CROSSVAL']}",
            background="gray15",
            foreground="white",
        ).grid(row=4, column=10, padx=10, sticky="w")
    ...
    def getSelectedValues(self):
        ...
        selectedValues = [
            self.selectedWeaponType.get(),
            self.selectedCase.get(),
            self.selectedRarity.get(),
            self.selectedWear.get(),
            self.selectedStatTrak.get(),
        ]
